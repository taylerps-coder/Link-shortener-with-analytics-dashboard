...
import { getClickTimeline, getClicksByCountry, getClicksByDevice } from '@linksnap/db';
...
// GET /api/links/[slug]/analytics/timeline
export async function GET(
  ...
  const interval = (url.searchParams.get('interval') ?? 'day') as 'hour' | 'day' | 'week';

  const data = await getClickTimeline(link.id, from, to, interval);
